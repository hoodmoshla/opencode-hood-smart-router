...
import { APP_IDS, APP_NAMES, CHANNEL, PROTOCOL_SCHEME } from "./constants"
...
const isHoodDeepLink = (url: string) => url.startsWith(`${PROTOCOL_SCHEME}://`)
...
const main = Effect.gen(function* () {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter(isHoodDeepLink)
    if (urls.length) {
  ...
  app.on("open-url", (event: Event, url: string) => {
    ...
    if (isHoodDeepLink(url)) emitDeepLinks([url])
  })
